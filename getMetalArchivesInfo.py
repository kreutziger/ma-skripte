...
QUERY_STRINGS = {
	'album': 'http://www.metal-archives.com/search/ajax-album-search/?field=name&query=',
	'band': 'http://www.metal-archives.com/search/ajax-band-search/?field=name&query=',
	'song': 'http://www.metal-archives.com/search/ajax-song-search/?field=title&query='
	}

#uebergebene argumenten werden in listen abgelegt, default argumente direkt als string
...
QUERY_REGEX = '">.*</a'
ERROR_MSG = 'error'
METAL_ARCHIVES_QUERY = QUERY_STRINGS.get(TYPE)
TYPE_STRING = {
...
for data in dataToSearch:
	...
	response = urlopen(METAL_ARCHIVES_QUERY + quote_plus(data))
	...
	if (dataCount == 0):
		...
		while (choice == 'n'):
			...
			if (choice.find(' ') == -1):
				if (choice.isdigit() and int(choice) <= len(dataData) and int(choice) > 0):
				...
				for number in numbers:
					if (number.isdigit() and int(number) <= len(dataData) and int(number) > 0):
